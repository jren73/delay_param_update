...
class FP16_DeepSpeedZeroOptimizer(object):
    ...
    def __init__(self,
        ...
        self.delay_param_update = False
        self.delay_params_update = delay_param_update
        self.global_steps = 0
    ...
    def async_inplace_copy_grad_to_fp32_buffer_from_gpu(self, param):
        ...
        #jie: copy p32 to p16
        if self.delay_param_update:
            src_tensor = self.single_partition_of_fp32_groups[i].view(-1).narrow(
                                        0,
                                        dest_offset,
                                        num_elements)
            dest_tensor = param.view(-1).narrow(0, source_offset, num_elements)
            dest_tensor.data.copy_(src_tensor, non_blocking=True)
    ...
    def step(self, closure=None):
        ...
        if self.delay_param_update is False and self.delay_params_update and self.global_steps > 20:
            self.delay_param_update = True
        if self.delay_param_update:
            return
        timers('optimizer_gradients').start()
        norm_groups = []
        ...
        if not self.cpu_offload:
            for group in self.single_partition_of_fp32_groups:
                group.grad = None
        if not self.delay_param_update:
            for fp16_partitions, fp32_partition in zip(self.parallel_partitioned_fp16_groups, self.single_partition_of_fp32_groups):
                fp16_partitions[partition_id].data.copy_(fp32_partition.data)
    ...
    def step_with_param_delay(self, closure=None):
        """
        Not supporting closure.
        """
        if self.delay_param_update is False:
            return
    ...
    def backward(self, loss, retain_graph=False):
        ...
        #if self.delay_param_update and self.global_steps != 20:
        #    with torch.cuda.stream(self.cpu_computation_stream):
        #        self.step_with_param_delay()
